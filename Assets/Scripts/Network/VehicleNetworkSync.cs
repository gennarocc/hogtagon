...
public class VehicleNetworkSynchronizer : NetworkBehaviour
{
    ...
    private void OnGUI()
    {
        if (vehicleController.IsDebugUIEnabled())
        {
            // Draw network debug info
            GUILayout.BeginArea(new Rect(10, 200, 200, 130));
            GUILayout.Label($"Role: {(IsOwner ? "Owner" : "Remote")}, Server: {IsServer}");
            GUILayout.Label($"Current Tick: {currentTick}");
            GUILayout.Label($"Last Processed: {lastProcessedInputTick}");
            GUILayout.Label($"Input Buffer: {inputBuffer.Count}/{inputBufferSize}");
            
            if (!IsOwner)
            {
                float posError = Vector3.Distance(transform.position, targetState.position);
                GUILayout.Label($"Interpolation: {(interpolationTime / networkInterpolationTime):F2}");
                GUILayout.Label($"Position Error: {posError:F2}m");
            }
